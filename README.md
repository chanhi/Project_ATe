...
> **담당 범위** — Redis/Celery 작업 큐, WebSocket 실시간 통신, CI/CD 연동 API
> 팀: **경기대학교 소프트웨어경영대학 / TQ7** (산학협력: NHN CLOUD)
...
- **CI/CD 자동 트리거** — GitHub Actions, Jenkins, GitLab CI 등과 Webhook 연동
- **Step DSL 지원** — DB 등록 없이 JSON 한 번으로 즉시 실행
- **Docker 격리 실행** — 모든 테스트는 격리된 컨테이너 내부에서 수행
- **Self-healing** — UI 변경 시 테스트 코드 자동 수정 (AI 엔진 협력)
...
celery -A main.celery_app worker --loglevel=info -Q test_execution,ci_cd,default
```
...
│   │
│   ├── api/                    # REST 엔드포인트
│   │   ├── scenarios.py        #   /scenarios  (AI팀 진입점)
│   │   ├── test_execution.py   #   /tests/*    (실행 + 즉시 실행)
│   │   └── ci_cd.py            #   /cicd/*     (CI/CD 연동)
│   │
│   ├── core/                   # 인프라
...
│   │   └── ci_cd.py            #   Webhook 콜백 + Allure 리포트
│   │
...
```json
{
  ...
  "ai_validation": {
    "is_valid": true,
    "retry_count": 1,
    "message": "Self-healing applied to correct CSS selector."
  }
}
```
...
```json
POST /api/v1/cicd/webhooks
{
  "project_id": "proj-001",
  "provider": "github_actions",
  "webhook_url": "https://your-ci.com/ate-callback",
  "trigger_on": ["push", "pull_request"],
  "secret_token": "your-hmac-secret",
  "scenario_ids": ["scen-abc", "scen-xyz"]
}
```

응답에서 `trigger_url`을 받아 CI/CD Webhook URL로 등록한다.

### 2. GitHub Actions 연동

```yaml
# .github/workflows/ate.yml
name: ATe E2E Tests
...
jobs:
  trigger-ate:
    ...
    steps:
      - name: Trigger ATe
        run: |
          curl -X POST https://your-ate.com/api/v1/cicd/trigger/github \
            -H "Content-Type: application/json" \
            -H "X-GitHub-Event: ${{ github.event_name }}" \
            -H "X-Hub-Signature-256: sha256=$(echo -n '${{ toJson(github.event) }}' \
                | openssl dgst -sha256 -hmac '${{ secrets.ATE_SECRET }}' \
                | awk '{print $2}')" \
...
pipeline {
  stages {
    stage('ATe E2E') {
      steps {
        httpRequest(
          url: 'https://your-ate.com/api/v1/cicd/trigger/jenkins',
          httpMode: 'POST',
...
### 4. 범용 Webhook (GitLab CI, CircleCI 등)

```json
POST /api/v1/cicd/trigger/custom
{
  "project_id": "proj-001",
  "scenario_ids": ["scen-abc", "scen-xyz"],
...
```
GET /api/v1/cicd/events/{event_id}
```

모든 트리거된 test_run의 상태를 묶어 `overall_status`
(`SUCCESS` | `FAILED` | `IN_PROGRESS`)로 반환.
...
| `WEBHOOK_SECRET` | 기본 Webhook HMAC 시크릿 | - |
