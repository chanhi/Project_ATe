...
async def generate_test_cases(request: TestCaseGenerateRequest):
    ...
    **지원 기법 (블랙박스):**
    ...
    # ── 각 기법별 빈 케이스 생성 ──
    # 실제로는 여기서 AI 엔진을 호출하지만, AI팀이 나중에 PUT으로 채워넣는 구조.
    #
    # AI팀에게 전달할 payload (참고):
    # {
    #   "source_text": source_text,
    #   "techniques": request.techniques,
    #   "project_context": {"name": project["name"], "base_url": project["base_url"]}
    # }
    ...
    for technique in request.techniques:
        ...
        generated_ids.append(test_case_id)
    ...
    return APIResponse(
        ...
        data={
            ...
            "target_urls": target_urls,
            "ai_hint": {
                "endpoint_to_update": "/test-cases/{test_case_id}",
                "source_text_length": len(source_text),
            },
        },
        message=f"{len(generated_ids)}개의 테스트 케이스가 생성되었습니다. AI팀이 PUT /test-cases/{{id}}로 내용을 채워넣어야 합니다.",
    )
