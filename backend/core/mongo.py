...
async def _create_indexes() -> None:
    ...
    # webhook_configs (기존 유지)
    await db.webhook_configs.create_index("project_id")

    # webhook_events (기존 유지)
    await db.webhook_events.create_index([("created_at", -1)])
    await db.webhook_events.create_index("webhook_config_id")
...
def get_webhook_configs_collection():
    return get_db().webhook_configs
